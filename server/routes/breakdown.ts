...
router.post('/', async (req, res, next) => {
  try {
    const { script, projectId } = req.body as { script: string; projectId?: string };
    if (!script?.trim()) {
    ...
    if (projectId) {
      ...
      if (ctx?.keyword) {
        contextNote = `\n\n【项目背景】该剧本围绕"${ctx.keyword}"主题创作${ctx.topicInsight ? `，核心方向：${ctx.topicInsight.slice(0, 100)}` : ''}。分镜描述请与此主题保持一致。`;
      }
    }
    ...
    const upstream = await fetch(`${baseUrl}/v1/chat/completions`, {
      ...
      body: JSON.stringify({
        model: 'deepseek-chat',
        messages: [
          { role: 'system', content: BREAKDOWN_SYSTEM_PROMPT + contextNote },
    ...
    if (projectId && scenes.length > 0) {
      upsertProjectContext(projectId, {
        scriptSummary: script.slice(0, 300),
        sceneCount: scenes.length,
        sceneDescriptions: scenes.map(s => `${s.shotType}：${s.description}`).join('\n'),
      });
    }
