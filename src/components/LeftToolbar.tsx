...
export default function LeftToolbar({
  activeTool, showAssets, showHistory, showAssistant,
  onToolChange, onToggleAssets, onToggleHistory, onToggleAssistant,
}: Props) {
  return (
    <div className="absolute left-4 top-1/2 -translate-y-1/2 z-50 flex flex-col items-center gap-1 bg-[#1a1a1a] border border-white/[0.08] rounded-2xl p-1.5 shadow-xl">
      ...
      <button
        ...
        title="画布助手"
        className={`w-9 h-9 flex items-center justify-center rounded-xl transition-colors text-base ${
          showAssistant
            ? 'bg-[#7c3aed]/30 text-[#a78bfa] ring-1 ring-[#7c3aed]/40'
            : 'text-gray-400 hover:bg-white/10 hover:text-gray-200'
        }`}
      >
        ✦
